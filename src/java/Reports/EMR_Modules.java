 ...
 * @author mwamb
 */
public class EMR_Modules extends HttpServlet {
    HttpSession session;
    String start_date,end_date,user_id,sheet_name,query,facility_ids,user_level;
    int row;
...
//    stborder.setWrapText(true);
      ...
      String get_queries = "SELECT id,query,report_name FROM other_reports where status=1";
      conn.rs = conn.st.executeQuery(get_queries);
      while(conn.rs.next()){
          query = conn.rs.getString(2).replace("start_date", "'"+start_date+"'").replace("end_date", "'"+end_date+"'").replace("facility_ids", facility_ids);

//          System.out.println("query is : "+query);
         
//          create sheet
          XSSFSheet sheet= wb.createSheet(conn.rs.getString(3));
...
System.out.println("Data is :"+conn.rs1.getString(i+1));
      ...
      if( conn.conn!=null){conn.conn.close();}
       ...
       String filename="Other_Daily_Data_Tracking_Reports_Genrated_on_"+mg.get_timestamp_string()+".xlsx";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(EMR_Modules.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
