...
public class load_view_edit extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      conn.st.execute("SET SESSION group_concat_max_len = 5000");
      String query ="";
      
      if(unique_key.equals("")){
      query ="SELECT \n" +
                        "entry_key," +
                        "GROUP_CONCAT(\" \"," +
                        "CONCAT(\"<b> \",q.question,\"</b>: <i class='data-content'>&nbsp;\",coalesce(" +
                        "IF(q.input_type_id in(3,4),IF(o.question_id=113,f.name,vl.name),numeric_value),text_value " +
                        "),\"&nbsp;</i>\") ORDER BY q.ordering_num ASC) as q_a," +
                        ...
                        + "LEFT OUTER JOIN facilities f ON o.numeric_value=f.id " +
                        " group by entry_key order by q.ordering_num ASC";
       ...
       conn.pst.setString(3, indicator_id);
          ...
          query ="SELECT \n" +
                        ...
                        + "i.multiple_entries " +
                        " FROM observations o " +
                        " INNER JOIN questions q ON o.question_id=q.id and o.entry_key in(SELECT entry_key FROM observations o  INNER JOIN questions q ON o.question_id=q.id and o.facility_id=? and (o.numeric_value=?  || o.text_value=? ) and q.indicator_id=? )"
                         + " INNER JOIN indicators i ON q.indicator_id=i.id " +
                        " LEFT OUTER JOIN value_labels vl ON o.numeric_value=vl.id "
                        + "LEFT OUTER JOIN facilities f ON o.numeric_value=f.id " +
                        " group by entry_key order by q.ordering_num ASC";
       ...
       conn.pst = conn.conn.prepareStatement(query);
       conn.pst.setString(1, facility_id);
       conn.pst.setString(2, unique_key);
       conn.pst.setString(3, unique_key);
       conn.pst.setString(4, indicator_id);      
