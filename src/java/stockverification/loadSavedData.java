...

{
    ...
    String qry=" "
            + "select * from stocks_data sd "
            + "left join stocks_commodities sc on sd.commodity= sc.id "
            + "left join facilities f on f.id = sd.facility "
            + "where sd.id='"+id+"'  ";
    ...
    String qry=" "
            + "select ifnull(reportingdate,'') as reportingdate, sd.id as id, sd.facility as facility, "
            + "f.name as facility_name,"
            + "batchno  , contacts , ifnull(contact_name,'Not indicated') as contact_name ,"
            + "sc.code as code,sd.packsize as packsize, delnoteno, qtyrec , pipeline from "
            + "stocks_data sd "
            + "left join stocks_commodities sc on sd.commodity= sc.id "
            + "left join facilities f on f.id = sd.facility ";
            ...
            jo.put("facility", conn.rs.getString("facility"));
            jo.put("facilityname", conn.rs.getString("facility_name"));
            jo.put("contact_name", conn.rs.getString("contact_name"));
